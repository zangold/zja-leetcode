    ...
    fn add_helper(l1: Option<Box<ListNode>>, l2: Option<Box<ListNode>>, carry: i32) -> Option<Box<ListNode>> {
        match (l1, l2) {
            (Some(n1), Some(n2)) => {
                ...
                Some(Box::new(ListNode { val: sum % 10, next: Solution::add_helper(n1.next, n2.next, sum / 10) }))
            },
            (Some(n), None) | (None, Some(n)) => {
                let sum = n.val + carry;
                Some(Box::new(ListNode { val: sum % 10, next: Solution::add_helper(n.next, None, sum / 10) }))
            },
            (None, None) if carry != 0 => {
...
fn list_from_slice(digits: &[i32]) -> Option<Box<ListNode>> {
    match digits.len() {
        ...
        _ => Some(Box::new(ListNode::new_next(digits[0], list_from_slice(&digits[1..])))),
    }
}
